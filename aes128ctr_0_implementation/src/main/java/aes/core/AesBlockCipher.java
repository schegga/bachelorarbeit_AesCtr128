...
public class AesBlockCipher {
    ...
    // Look-up tables for T-Table implementation of AES (byte to Big Endian int)
...
//--------------------------Functions for generating the round keys 128 bits---------------------------------//
   /**
    * Expands the given key to the round keys for the AES encryption algorithm
    * @param key the key to be expanded
    * @return the round keys as int array
    */
    public int[] expandKey128(byte[] key) {
        int[] roundKeys = new int[44];
    
        // first round key is the key itself (byte to Big Endian int)
        for (int i = 0; i < 4; i++) {
        ...
        for (int i = 1; i < 11; i++) {
            int lastWord = roundKeys[(i - 1) * 4 + 3];
    
            // shift the last word 8 bits to the left
            lastWord = circularLeftShift(lastWord, 8);
    
            // apply the S-Box to each byte of the last word
            int lookUp1 = lastWord & 255;
            int lookUp2 = (lastWord >>> 8) & 255;
            int lookUp3 = (lastWord >>> 16) & 255;
            int lookUp4 = (lastWord >>> 24) & 255;
    
            lastWord = (S[lookUp1] & 255) | 
                       ((S[lookUp2] & 255) << 8) | 
                       ((S[lookUp3] & 255) << 16) | 
                       ((S[lookUp4] & 255) << 24);
    
            // XOR the last word with the rcon
            lastWord ^= (rcon[i-1] & 255) << 24; // Only the first byte should be XORed with rcon
    
            // roundkeys
            roundKeys[i * 4] = roundKeys[(i - 1) * 4] ^ lastWord;
            roundKeys[i * 4 + 1] = roundKeys[(i - 1) * 4 + 1] ^ roundKeys[i * 4];
            roundKeys[i * 4 + 2] = roundKeys[(i - 1) * 4 + 2] ^ roundKeys[i * 4 + 1];
            roundKeys[i * 4 + 3] = roundKeys[(i - 1) * 4 + 3] ^ roundKeys[i * 4 + 2];
        }
        return roundKeys;
    }
    
    /**
     * Makes a circular left shift of the given integer x by the given shift in bits
     * @param x the integer to be shifted
     * @param shift the amount of bits to be shifted
     * @return the shifted integer
     */
    public int circularLeftShift(int x, int shift) {
        x = (x << shift) | (x >>> (32 - shift)); // << is left shift, >>> is unsigned right shift, | is bitwise or
        return x;
    }
    
//--------------------------Function for one block AES-encryption with 128-bit roundkeys---------------------------------//
    /**
     * Encrypts one block of 128 bits with the given round keys
     * @param input the block to be encrypted
     * @param roundKeys the round keys to be used
     * @return the encrypted block
     */ 
    public byte[] blockEncryption128(int[] input, int[] roundKeys) {
        int[] state = new int[4];
        System.arraycopy(input, 0, state, 0, 4);
        ...
        for (int round = 1; round < 10; round++) {
            ...
            x2 = (state[2] >>> 8) & 0xFF;
            x3 = state[3] & 0xFF;
            stateTemp[0] = T0[x0] ^ T1[x1] ^ T2[x2] ^ T3[x3] ^ roundKeys[round * 4];
    
            x0 = (state[1] >>> 24) & 0xFF; 
            x1 = (state[2] >>> 16) & 0xFF; 
            x2 = (state[3] >>> 8) & 0xFF; 
            x3 = state[0] & 0xFF;
            stateTemp[1] = T0[x0] ^ T1[x1] ^ T2[x2] ^ T3[x3] ^ roundKeys[round * 4 + 1];
    
            x0 = (state[2] >>> 24) & 0xFF; 
            x1 = (state[3] >>> 16) & 0xFF; 
            x2 = (state[0] >>> 8) & 0xFF; 
            x3 = state[1] & 0xFF;
            stateTemp[2] = T0[x0] ^ T1[x1] ^ T2[x2] ^ T3[x3] ^ roundKeys[round * 4 + 2];
    
            x0 = (state[3] >>> 24) & 0xFF; 
            x1 = (state[0] >>> 16) & 0xFF; 
            x2 = (state[1] >>> 8) & 0xFF; 
            x3 = state[2] & 0xFF;
            stateTemp[3] = T0[x0] ^ T1[x1] ^ T2[x2] ^ T3[x3] ^ roundKeys[round * 4 + 3];
    
            System.arraycopy(stateTemp, 0, state, 0, 4);
        }
        ...
        x3 = state[3] & 0xFF;
        // for int[] output:
        // stateTemp[0] = ((S[x0] & 0xFF) ^ (roundKeys[40] >>> 24))  << 24 | ((S[x1] & 0xFF) ^ ((roundKeys[40] >>> 16) & 0xFF)) << 16 | 
        //             ((S[x2] & 0xFF) ^ ((roundKeys[40] >>> 8) & 0xFF) ) << 8 | (S[x3] & 0xFF) ^ (roundKeys[40] & 0xFF); 
        ciphertext[i++] = (byte) (S[x0] ^ (roundKeys[40] >>> 24));
        ciphertext[i++] = (byte) (S[x1] ^ ((roundKeys[40] >>> 16) & 0xFF));
        ciphertext[i++] = (byte) (S[x2] ^ ((roundKeys[40] >>> 8) & 0xFF));
        ciphertext[i++] = (byte) (S[x3] ^ (roundKeys[40] & 0xFF));
        ...
        x3 = state[0] & 0xFF;
        // for int[] output:
        // stateTemp[1] = ((S[x0] & 0xFF) ^ (roundKeys[41] >>> 24))  << 24 | ((S[x1] & 0xFF) ^ ((roundKeys[41] >>> 16) & 0xFF)) << 16 | 
        //             ((S[x2] & 0xFF) ^ ((roundKeys[41] >>> 8) & 0xFF) ) << 8 | (S[x3] & 0xFF) ^ (roundKeys[41] & 0xFF);
        ciphertext[i++] = (byte) (S[x0] ^ (roundKeys[41] >>> 24));
        ciphertext[i++] = (byte) (S[x1] ^ ((roundKeys[41] >>> 16) & 0xFF));
        ciphertext[i++] = (byte) (S[x2] ^ ((roundKeys[41] >>> 8) & 0xFF));
        ciphertext[i++] = (byte) (S[x3] ^ (roundKeys[41] & 0xFF));
        ...
        x3 = state[1] & 0xFF;
        // for int[] output:
        // stateTemp[2] = ((S[x0] & 0xFF) ^ (roundKeys[42] >>> 24))  << 24 | ((S[x1] & 0xFF) ^ ((roundKeys[42] >>> 16) & 0xFF)) << 16 | 
        //             ((S[x2] & 0xFF) ^ ((roundKeys[42] >>> 8) & 0xFF) ) << 8 | (S[x3] & 0xFF) ^ (roundKeys[42] & 0xFF);
        ciphertext[i++] = (byte) (S[x0] ^ (roundKeys[42] >>> 24));
        ciphertext[i++] = (byte) (S[x1] ^ ((roundKeys[42] >>> 16) & 0xFF));
        ciphertext[i++] = (byte) (S[x2] ^ ((roundKeys[42] >>> 8) & 0xFF));
        ciphertext[i++] = (byte) (S[x3] ^ (roundKeys[42] & 0xFF));
        ...
        x3 = state[2] & 0xFF;
        // for int[] output:            
        // stateTemp[3] = ((S[x0] & 0xFF) ^ (roundKeys[43] >>> 24))  << 24 | ((S[x1] & 0xFF) ^ ((roundKeys[43] >>> 16) & 0xFF)) << 16 | 
        //             ((S[x2] & 0xFF) ^ ((roundKeys[43] >>> 8) & 0xFF) ) << 8 | (S[x3] & 0xFF) ^ (roundKeys[43] & 0xFF);
        ciphertext[i++] = (byte) (S[x0] ^ (roundKeys[43] >>> 24));
        ciphertext[i++] = (byte) (S[x1] ^ ((roundKeys[43] >>> 16) & 0xFF));
        ciphertext[i++] = (byte) (S[x2] ^ ((roundKeys[43] >>> 8) & 0xFF));
        ciphertext[i++] = (byte) (S[x3] ^ (roundKeys[43] & 0xFF));
